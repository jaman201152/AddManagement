<?php
    ...
    $where = " order_id like '$productid%' and ref_id like '$q' ";
...
// Row Count End.
    ...
    $rs1 = $con->prepare("select * from tbl_order 
     where " . $where . " order by order_id DESC limit $offset,$rows");
    $rs1->execute();
    $items = array();
    $size = array();
    while($row =$rs1->fetch(PDO::FETCH_ASSOC)){
            
            $rs1_customer = $con->prepare("select * from tbl_reference where ref_id='$q' ");
    ...
    while($row_customer =$rs1_customer->fetch(PDO::FETCH_ASSOC)){
                ...
                $date=$d->format('d-M-Y'); // date show like d-m-Y.
                $row['order_date']=$date;
        ...
        $cust_order1= array_merge($cust_order,$size); // two array has been merge.
        array_push($items, $cust_order1);
    ...
    $payable_amount1->execute();
        ...
        while($row=$payable_amount1->fetch(PDO::FETCH_ASSOC)){
            ...
            foreach ($row as $row3)
             ...
             $discount1=$con->prepare("select sum(discount_amount) as discount_amount from tbl_order where " . $where);
    $discount1->execute();
         $dis2 = array();
        while($dis=$discount1->fetch(PDO::FETCH_ASSOC)){
            
           foreach($dis as $dis1)
               $dis2['discount_amount'] = number_format($dis1,2,'.',','); // display discount_amount in 2 decimal value.
